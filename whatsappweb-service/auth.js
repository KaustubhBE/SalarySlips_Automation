...
class WhatsAppAuth extends EventEmitter {
    constructor(clientId = 'default') {
        ...
        this._lastConnectionStatusCheck = 0;
        
        console.log(`WhatsAppAuth initialized for clientId: ${this.clientId}`);
        console.log(`Session path: ${this.sessionPath}`);
    }

    // Enhanced method to clean up corrupted session files and lock files
    async cleanupCorruptedSession() {
        console.log(`Checking for corrupted session files for clientId: ${this.clientId}`);
        ...
        for (const lockFile of lockFiles) {
            const lockFilePath = path.join(this.sessionPath, lockFile);
            try {
                await fs.access(lockFilePath, fs.constants.F_OK);
                await fs.unlink(lockFilePath);
                console.log(`Successfully deleted lock file: ${lockFile}`);
                cleanedFiles++;
            } catch (error) {
        ...
        // Also clean up Chrome crash dumps
        try {
            const crashDumpsDir = path.join(this.sessionPath, 'Crashpad');
        ...
        if (cleanedFiles > 0) {
            console.log(`Cleaned up ${cleanedFiles} corrupted session files for ${this.clientId}`);
        } else {
            console.log(`No corrupted session files found for ${this.clientId}`);
        }
    }

    // Force cleanup of lock files (more aggressive than regular cleanup)
    async forceCleanupLockFiles() {
        console.log(`Force cleaning up lock files for clientId: ${this.clientId}`);
        ...
        for (const lockFile of lockFiles) {
            const lockFilePath = path.join(this.sessionPath, lockFile);
            try {
                // Force delete without checking if file exists
                await fs.unlink(lockFilePath).catch(() => {}); // Ignore errors
                console.log(`Force deleted lock file: ${lockFile}`);
                cleanedFiles++;
            } catch (error) {
                // Ignore all errors for force cleanup
            }
        }

        // Also force clean up Chrome crash dumps
        try {
            const crashDumpsDir = path.join(this.sessionPath, 'Crashpad');
            await fs.rm(crashDumpsDir, { recursive: true, force: true }).catch(() => {}); // Ignore errors
            console.log('Force cleaned up crash dumps directory');
            cleanedFiles++;
        } catch (error) {
            // Ignore errors
        }

        console.log(`Force cleaned up ${cleanedFiles} lock files for ${this.clientId}`);
    }
    ...
    async _performInitialization() {
        ...
        while (retryCount < maxRetries) {
            try {
                console.log(`Initializing WhatsApp client for clientId: ${this.clientId} (attempt ${retryCount + 1}/${maxRetries})`);
                
                // Clean up lock files before each attempt
                await this.cleanupCorruptedSession(); 
                
                // Force cleanup of lock files before each attempt to prevent SingletonLock issues
                await this.forceCleanupLockFiles();
                
                // Create and initialize client in one go to catch all errors
            ...
            if (this.isReady) {
                ...
                console.error(`Error during client initialization (attempt ${retryCount + 1}/${maxRetries}):`, error.message);
                ...
                if (error.message.includes('SingletonLock') || error.message.includes('ProcessSingleton')) {
                    console.log(`SingletonLock error detected, cleaning up and retrying...`);
                    
                    // Force cleanup of lock files
                    await this.forceCleanupLockFiles();
                ...
                // If not a lock file error or max retries reached, throw the error
                this.isInitialized = false;
                throw error;
            }
        }
        ...
        throw new Error(`Failed to initialize WhatsApp client after ${maxRetries} attempts`);
    }
    ...
    setupEventListeners() {
        ...
        this.client.on('authenticated', async () => {
            ...
            try {
                await this.waitForWhatsAppStore(60000);
            } catch (error) {
                console.warn(`[TIMING] Store object wait failed: ${error.message}`);
            }
    ...
    async waitForWhatsAppStore(maxWaitMs = null) {
        ...
        const maxWait = maxWaitMs || Infinity;
    ...
    async handleAuthFailure() {
        try {
            ...
            await this.cleanupCorruptedSession(); // Added cleanup
    ...
    async triggerLogin(timeoutMs = null) {
        try {
            ...
            // Check for QR code immediately after initialization
            if (this.currentQR) {
                console.log(`QR already available for ${this.clientId} after initialization, returning it`);
                return {
                    isReady: false,
                    qr: this.currentQR,
                    authenticated: false
            ...
            // If no QR code yet, wait a bit for it to be generated
            console.log(`No QR code available yet for ${this.clientId}, waiting for QR generation...`);
            
            // Give a small delay for QR code generation
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Check again after delay
            if (this.currentQR) {
                console.log(`QR code available after delay for ${this.clientId}, returning it`);
                return {
                    isReady: false,
                    qr: this.currentQR,
                    authenticated: false
    ...
    async checkConnectionStatus() {
        try {
            ...
            const throttleInterval = 5 * 60 * 1000;
        ...
        try {
            ...
            if (status.authenticated && status.userInfo) {
                return {
                    ...
                    phoneNumber: status.userInfo.phoneNumber
                };
            }
            return null;
        } catch (error) {
