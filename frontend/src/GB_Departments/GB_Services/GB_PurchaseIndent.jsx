...
const useSessionManagement = () => {
  ...
  const registerSession = (orderId) => {
    try {
      const activeSessions = JSON.parse(localStorage.getItem('gb_active_sessions') || '{}')
      activeSessions[sessionId] = {
        timestamp: Date.now(),
        orderId: orderId
      }
      localStorage.setItem('gb_active_sessions', JSON.stringify(activeSessions))
    } catch (error) {
...
const hasEditSpecOptions = useMemo(
  ...
  // Show detailed log report similar to GB_ReactorReports
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const completedOrders = JSON.parse(localStorage.getItem('gb_completed_orders') || '[]')
          ...
          localStorage.setItem('gb_completed_orders', JSON.stringify(completedOrders))
        } catch (error) {
        ...
        if (notificationMethod) {
          try {
            ...
            if (!sheetId) {
              alert('❌ Error: No sheet ID found for Gulbarga plant configuration')
              // Reset order ID for next order
              setOrderIdGenerated(false)
              setOrderId('')
              return
  ...
  return (
    <div className="po-purchase_indent-container">
      ...
      <BackButton label="Back to Store" to="/gulbarga/gb_store" />
...
export default GB_PurchaseIndent
