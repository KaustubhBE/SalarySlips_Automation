...
function Processing({ mode = 'single' }) {
  ...
  const hasProcessingPermission = () => {
    ...
    const requiredPermission = mode === 'single' ? 'gb_single_processing' : 'gb_batch_processing';
    return hasPermission(requiredPermission, 'gulbarga', 'humanresource');
  };
      ...
      <Routes>
        ...
        <Route path="" element={
          <div className="input-elements">
            ...
            <BackButton label="Back to Department" to="/gulbarga/gb_humanresource" />
            {mode === 'single' && (
              <>
