...
const generateFallbackOrderId = () => {
  ...
  return `HB_${month.toString().padStart(2, '0')}${year.toString().padStart(2, '0')}-${(timestamp % 10000).toString().padStart(4, '0')}`
}
...
const useSessionManagement = () => {
  ...
  const registerSession = (orderId) => {
    try {
      const activeSessions = JSON.parse(localStorage.getItem('hb_active_sessions') || '{}')
      activeSessions[sessionId] = {
        timestamp: Date.now(),
        orderId: orderId
      }
      localStorage.setItem('hb_active_sessions', JSON.stringify(activeSessions))
    } catch (error) {
...
const HB_PlaceOrder = () => {
  ...
  const fetchAuthorityList = async () => {
    try {
      ...
      // Find the HB plant data to get the sheet ID
      const hbPlant = PLANT_DATA.find(plant => plant.document_name === 'HB')
      const sheetId = hbPlant?.material_sheet_id
      
      if (!sheetId) {
        console.error('No sheet ID found for HB plant')
        return
      }
