...
const fetchMaterialUomFromBackend = async (category, subCategory, specifications, materialName) => {
  try {
    const currentPlant = PLANT_DATA.find(plant => plant.document_name === 'OM')
...
const focusFieldWithError = (primaryField, fieldsToHighlight = [primaryField]) => {
  ...
  const fetchAuthorityList = async () => {
    try {
      ...
      // Find the OM plant data to get the sheet ID
      const currentPlant = PLANT_DATA.find(plant => plant.document_name === 'OM')
      const sheetId = currentPlant?.material_sheet_id
      
      if (!sheetId) {
        console.error('No sheet ID found for OM plant')
        return
      }
...
const hasEditSpecOptions = useMemo(
  ...
  // Show detailed log report similar to OM_ReactorReports
      ...
      if (response.data.success) {
        ...
        if (notificationMethod) {
          try {
            ...
            if (!sheetId) {
              alert('❌ Error: No sheet ID found for OM plant configuration')
              // Reset order ID for next order
              setOrderIdGenerated(false)
              setOrderId('')
              return
...
export default OM_PurchaseIndent
