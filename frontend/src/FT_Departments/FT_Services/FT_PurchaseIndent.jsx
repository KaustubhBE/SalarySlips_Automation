...
const generateFallbackOrderId = () => {
  ...
  return `FT_${month.toString().padStart(2, '0')}${year.toString().padStart(2, '0')}-${(timestamp % 10000).toString().padStart(4, '0')}`
}
...
const useSessionManagement = () => {
  ...
  const registerSession = (orderId) => {
    try {
      const activeSessions = JSON.parse(localStorage.getItem('ft_active_sessions') || '{}')
      activeSessions[sessionId] = {
        timestamp: Date.now(),
        orderId: orderId
      }
      localStorage.setItem('ft_active_sessions', JSON.stringify(activeSessions))
    } catch (error) {
...
const hasEditSpecOptions = useMemo(
  ...
  // Show detailed log report similar to FT_ReactorReports
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const completedOrders = JSON.parse(localStorage.getItem('ft_completed_orders') || '[]')
          ...
          localStorage.setItem('ft_completed_orders', JSON.stringify(completedOrders))
        } catch (error) {
        ...
        if (notificationMethod) {
          try {
            ...
            if (!sheetId) {
              alert('❌ Error: No sheet ID found for Fertilizer plant configuration')
              // Reset order ID for next order
              setOrderIdGenerated(false)
              setOrderId('')
              return
...
export default FT_PurchaseIndent
