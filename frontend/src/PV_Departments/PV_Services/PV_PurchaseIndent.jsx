...
const generateFallbackOrderId = () => {
  ...
  return `PV_${month.toString().padStart(2, '0')}${year.toString().padStart(2, '0')}-${(timestamp % 10000).toString().padStart(4, '0')}`
}
...
const fetchMaterialUomFromBackend = async (category, subCategory, specifications, materialName) => {
  try {
    const currentPlant = PLANT_DATA.find(plant => plant.document_name === 'PV')
...
const useSessionManagement = () => {
  ...
  const registerSession = (orderId) => {
    try {
      const activeSessions = JSON.parse(localStorage.getItem('pv_active_sessions') || '{}')
      activeSessions[sessionId] = {
        timestamp: Date.now(),
        orderId: orderId
      }
      localStorage.setItem('pv_active_sessions', JSON.stringify(activeSessions))
    } catch (error) {
...
const focusFieldWithError = (primaryField, fieldsToHighlight = [primaryField]) => {
  ...
  const fetchAuthorityList = async () => {
    try {
      ...
      // Find the Pavagada plant data to get the sheet ID
      const currentPlant = PLANT_DATA.find(plant => plant.document_name === 'PV')
      const sheetId = currentPlant?.material_sheet_id
      
      if (!sheetId) {
        console.error('No sheet ID found for Pavagada plant')
        return
      }
...
const hasEditSpecOptions = useMemo(
  ...
  if (missingFields.length > 0) {
    ...
    setAddItemValidationError(`Please fill: ${missingLabels}`)
  ...
  // Show detailed log report similar to PV_ReactorReports
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const completedOrders = JSON.parse(localStorage.getItem('pv_completed_orders') || '[]')
          ...
          localStorage.setItem('pv_completed_orders', JSON.stringify(completedOrders))
        } catch (error) {
        ...
        if (notificationMethod) {
          try {
            ...
            if (!sheetId) {
              alert('❌ Error: No sheet ID found for Pavagada plant configuration')
              // Reset order ID for next order
              setOrderIdGenerated(false)
              setOrderId('')
              return
...
export default PV_PurchaseIndent
