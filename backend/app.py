...
from Utils.salary_slips_utils import process_salary_slip
...
def generate_salary_slips_batch():
    try:
        ...
        app.logger.info("Received request to generate salary slips")
        ...
        app.logger.info("Salary slips generated successfully!")
        return jsonify({"message": "Salary slips generated successfully!"}), 200
...
def generate_salary_slip_single():
    try:
        ...
        # Find the employee by name or code
        employee = next((emp for emp in employees if emp[4] == employee_identifier or emp[0] == employee_identifier), None)
        if not employee:
