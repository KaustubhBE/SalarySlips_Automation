...
import email.utils
from Utils.firebase_utils import get_user_by_email_with_metadata
import re
import base64
...
def send_email_oauth(user_email, recipient_email, subject, body, attachment_paths=None, cc=None, bcc=None):
    """
    Send email using OAuth (Gmail API) only.
    """
    # Use Gmail API only - no fallback
    return send_email_gmail_api(user_email, recipient_email, subject, body, attachment_paths, cc, bcc)